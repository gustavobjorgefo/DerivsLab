    ...
    def generate_paths(self):
        ...
        for t in range(1, self.steps + 1):
            ...
            paths1[:, t] = paths1[:, t - 1] * np.exp(
                (self.mu1 - 0.5 * self.sigma1**2) * self.dt + self.sigma1 * np.sqrt(self.dt) * Z1
            )

            paths2[:, t] = paths2[:, t - 1] * np.exp(
                (self.mu2 - 0.5 * self.sigma2**2) * self.dt + self.sigma2 * np.sqrt(self.dt) * Z2
            )
...
class MultiAssetGBMPathGenerator:
    ...
    def generate_paths(self) -> np.ndarray:
        ...
        for t in range(1, self.steps + 1):
            ...
            paths[:, :, t] = paths[:, :, t - 1] * np.exp(drift + diffusion_scale * correlated_Z)
...
if __name__ == "__main__":
    gbm_generator = GBMPathGenerator(
        S0=100.0, mu=0.05, sigma=0.25, T=0.50, steps=126, n_paths=10000, seed=None
    )
